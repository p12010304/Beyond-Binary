import { Calendar, Clock, MapPin, Trash2, Volume2 } from 'lucide-react'
    ...
    return (
      <div className={cn('text-center py-12', className)}>
        ...
        <p className="text-muted-foreground">
          No events found. Create one using the form{preferences.simplified_ui ? ' on the right' : ' above'}.
        </p>
      </div>
    )
  }
    ...
    <div className={cn('space-y-3', className)} role="list" aria-label="Calendar events">
      {events.map((event) => (
        <Card
          ...
          className={cn(
            'transition-colors',
            isToday(event.start) && 'border-primary/50 bg-primary/5',
          )}
          role="listitem"
        >
          <CardContent className="flex items-start gap-4 p-4">
            ...
            <div className="shrink-0 text-center min-w-[60px]">
              <p className="text-sm font-semibold">{formatTime(event.start)}</p>
